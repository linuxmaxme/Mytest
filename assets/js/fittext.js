      ...
      _tileOnLoad : function() {
        if (this._layer._isIE() == false) {
          ...
          if (ctx) {
            ...
            if (this._layer.options.maskColor && this._layer.options.masking) {
...
By default it will create a duplicate image thus you 
can set the responsive or the sizes via CSS
...
"use strict";
    ...
    if (self.hasClass('greyscale-processed') == true) {
      return false;
    }
	  ...
	  image.onload = function() {
  		...
  		self.wrap('<div class="'+ options.container +'" />');
  		self.parent('.'+ options.container +'').css({'position':'relative'});
  		self.parent('.'+ options.container +'').append('<canvas class="canv"></canvas>');		
  		
  		var cloneImage = new Image();
  		    cloneImage.src = self.attr('src');
  	  
  		var width = cloneImage.width;
  	  var height = cloneImage.height;
  	      
  		self.next('.canv').attr({"width": width ,"height": height});
  		var canvas = self.next('.canv').get(0);
  		...
  		var altImage = new Image();
  		    altImage.src = canvas.toDataURL();
  		$(altImage).addClass('canv');
  		$(canvas).remove();
  		self.parent('.'+ options.container +'').append(altImage); 
		
      self.next('.canv').css({'position':'absolute','top': self.position().top,'left': self.position().left,'bottom': self.position().top + self.outerHeight(),'right': self.position().left + self.outerWidth(),'height':'100%','z-index':10});
      self.css({'z-index':-1});

      
      //hover effect
      self.parent('.'+ options.container +'').hover( function() {
        $(this).children('.canv').css('opacity', 0);
        },function() {
        $(this).children('.canv').css('opacity', 1);
      });
      
      self.addClass('grayscale-processed');
	  };
  ...
  $(window)  
    .on('load', function() {
      ...
      setTimeout(function() {
        ...
        $('img.grayscale-hover:not(.grayscale-processed)').hoverizr({
          effect: 'grayscale'
        });
        
        $('img.blur-hover:not(.grayscale-processed)').hoverizr({
          effect: 'blur'
        });
        
        $('img.invert-hover:not(.grayscale-processed)').hoverizr({
          effect: 'invert'
        });
      ...
      // Wait till mask image loaded
        ...
        var img = new Image();
				...
				$(img).one('load', function() {
				  ...
				  // Create cloned image to get pure original width and height
				  cloneImage = new Image();
				  cloneImage.src = img.src;
				  
	        // Define variables and reset.
	        var $canvasObj = createCanvas($(img), cloneImage.width, cloneImage.height)[0];
...
jQuery(document).ready(function($) {
  "use strict";
  $(window).on('load', function() {
    $('.text-tailor').textTailor({fit: false});
  });
  
  // Integrate with isotope
  $(document).on('layoutComplete', function(isoInstance, item) {
    var textTailorElement = $(isoInstance.element).find('.text-tailor');
    if (textTailorElement.length) {
      setTimeout(function() {
        textTailorElement.textTailor({fit: false});
      }, 100);
    }
  })
});